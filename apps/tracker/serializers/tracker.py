...
class TrackerDetailProductModelSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        sum_quantity = TrackerDetailProductModel.objects.filter(tracker_detail=data.get('tracker_detail')).aggregate(Sum('quantity'))
        if sum_quantity.get('quantity__sum') and sum_quantity.get('quantity__sum') + quantity > data.get('tracker_detail').quantity:
...
class TrackerSerializer(serializers.ModelSerializer):
    ...
    tracker_detail_tracker = TrackerDetailModelSerializer(many=True, read_only=True)
