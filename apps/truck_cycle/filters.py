...
class PautaFilter(django_filters.FilterSet):
    ...
    date_after = django_filters.DateFilter(field_name='operational_date', lookup_expr='gte')
    date_before = django_filters.DateFilter(field_name='operational_date', lookup_expr='lte')
    ...
    class Meta:
        model = PautaModel
        fields = ['status', 'date_after', 'date_before', 'truck', 'transport_number', 'is_reload']
